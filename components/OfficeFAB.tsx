...
export default function OfficeFAB({ isGuest, requireAuth, onOpenItem }: OfficeFABProps) {
  ...
  const getIconForType = (type: string) => {
    switch (type) {
      case 'docs': return <FileText size={12} strokeWidth={2.5} className="text-blue-600" />;
  ...
  const getBorderClassForType = (type: string) => {
    switch (type) {
      case 'docs': return 'border-blue-200/60';
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 flex items-end gap-3" ref={menuRef}>
      {/* Floating Label */}
      <div
        className={`transition-all duration-500 ease-out mb-2 ${
          isOpen
            ? 'opacity-100 translate-x-0'
            : 'opacity-0 translate-x-4 pointer-events-none'
        }`}
      >
        <div className="bg-white/80 backdrop-blur-md rounded-full px-4 py-2 shadow-lg border border-white/30 animate-fade-in-scale">
      ...
      {/* Menu Items */}
      <div
        className={`absolute bottom-16 right-0 flex flex-col gap-2 transition-all duration-500 ease-out ${
          isOpen
      ...
      >
        {officeSuites.map((suite, index) => (
          <button
            ...
            className={`group flex items-center gap-3 bg-white/90 backdrop-blur-md rounded-2xl border-[1px] shadow-[0_8px_32px_-8px_rgba(0,0,0,0.08)] hover:shadow-[0_12px_40px_-8px_rgba(0,0,0,0.15)] hover:-translate-y-1 hover:bg-white transition-all duration-300 ${suite.borderClass} p-3 w-64 btn-press`}
            style={{
              animation: isOpen ? `slideUp 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) ${index * 0.05}s forwards` : 'none',
              opacity: 0,
              transform: 'translateY(12px) scale(0.96)',
            }}
          >
            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${suite.color} flex items-center justify-center text-white shadow-md group-hover:scale-110 transition-transform duration-300 flex-shrink-0`}>
              {suite.icon}
            </div>
            <div className="flex-1 text-left min-w-0">
              <div className="text-sm font-semibold text-gray-900 truncate group-hover:text-blue-600 transition-colors">{suite.name}</div>
              <div className="text-[11px] text-gray-500 font-medium">{suite.description}</div>
            </div>
            <ChevronRight size={14} strokeWidth={2.5} className="text-gray-300 group-hover:text-blue-600 group-hover:translate-x-1 transition-all duration-300 flex-shrink-0" />
          </button>
        ))}
        
        {/* Recent Documents */}
        <div className="bg-white/90 backdrop-blur-md rounded-2xl border-[1px] border-gray-200/60 shadow-[0_8px_32px_-8px_rgba(0,0,0,0.08)] p-3 w-64 animate-fade-in-scale">
          ...
          <div className="space-y-1.5">
            {loadingRecent ? (
              ...
              <div className="py-2">
                <span className="text-[11px] text-gray-400 font-medium italic">No recent items</span>
              </div>
            ) : (
              recentItems.map((item, idx) => (
                <button
                  ...
                  className={`flex items-center gap-2.5 w-full text-left py-2 px-2 rounded-xl hover:bg-gray-50/80 transition-all group border-[1px] border-transparent hover:${getBorderClassForType(item.type)} btn-press animate-slide-up`}
                  style={{ animationDelay: `${idx * 60}ms`, opacity: 0 }}
                >
                  {getIconForType(item.type)}
                  <span className="flex-1 text-xs font-medium text-gray-900 truncate group-hover:text-blue-600 transition-colors">{item.title || 'Untitled'}</span>
      ...
      {/* The FAB Button - Premium, Floaty, with Glow */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="relative flex items-center justify-center gap-2 px-5 py-3 rounded-full bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group focus:outline-none btn-press"
      >
        {/* Subtle glow behind the button */}
        <div className="absolute inset-0 rounded-full bg-gradient-to-r from-blue-600 to-purple-600 blur-xl opacity-40 group-hover:opacity-60 transition-opacity duration-300 -z-10" />
